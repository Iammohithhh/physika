...
# Active plane-wave mask and sample orbital coefficients.
ACTIVE = torch.tensor([True, True, True, False, True, False, False, False])
W_ACTIVE = torch.tensor([0 + 0j, 1 + 2j, -1 + 0j, 0 + 3j], dtype=torch.complex64)
...
def dens_ns():
    """Execute examples/dft_density.phyk."""
...
@pytest.fixture(scope="module")
def h_atom(dens_ns):
    """Single H atom on a 60×60×60 grid with a=16 Bohr."""
...
class TestMaskEmbed:
    ...
    # Verify values are placed at the active plane-wave indices.
    ...
    # Verify inactive grid points remain zero.
        values = torch.tensor([1 + 1j, 2 + 0j, 0 + 3j, -1 + 2j],
                              dtype=torch.complex64)
        out = dens_ns["mask_embed"](values, ACTIVE, N)
...
class TestOrth:
    ...
    # Compare against the reference normalization formula.
        Omega = float(h_like_atoms.volume())
        out = dens_ns["orth"](h_like_atoms, W_ACTIVE)
        expected = W_ACTIVE / torch.sqrt(torch.tensor(Omega) * (W_ACTIVE.abs() ** 2).sum())
        assert torch.allclose(out, expected, rtol=r_tol, atol=a_tol)

    def test_normalizes_to_unity(self, dens_ns, h_like_atoms):
    # Verify the normalized orbital has unit norm.
        Omega = float(h_like_atoms.volume())
        Y = dens_ns["orth"](h_like_atoms, W_ACTIVE)
...
class TestDensity:
    ...
    # Verify the total electron density integrates to the occupation number.
...
class TestHartreePotential:
    ...
    def test_g_zero_component_is_zero(self, dens_ns, h_like_atoms):
    # Verify the G = 0 component is zero.
        out = dens_ns["get_phi"](h_like_atoms, self.N_TEST)
        assert out[0] == 0
