...
# Small hydrogen-like system used for SCF unit tests.
S = (2, 2, 2)
A = 2.0
ECUT = 5.0

# Larger grid used for the end-to-end hydrogen test.
H_A = 16.0
H_ECUT = 16.0
H_S = 60
...
def h_like_atoms(scf_ns):
    """Create the small hydrogen-like system used in SCF unit tests."""
    return scf_ns["Atoms"](
        A,
        ECUT,
        S[0],
...
@pytest.fixture(scope="module")
def h_atom(scf_ns):
    """Create the larger-grid hydrogen system used for the end-to-end test."""
    return scf_ns["Atoms"](
        H_A,
        H_ECUT,
        H_S,
...
class TestInitW:
    """Tests for construction of the initial orbital."""

    def test_is_lowdin_normalized(self, scf_ns, h_like_atoms):
        # Check normalization of the initial orbital.
    ...
    def test_reproducible_with_same_seed(self, scf_ns, h_like_atoms):
        # The same seed should give the same orbital.
    ...
    def test_differs_with_different_seed(self, scf_ns, h_like_atoms):
        # Different seeds should give different orbitals.
...
class TestEnergyOfW:
    """Tests for evaluation of the total DFT energy."""

    def test_matches_manual_pipeline(self, scf_ns, h_like_atoms):
        # Compare energy_of_W with the individual energy terms.
        ...
        # Avoid zero density in the XC functionals.
        n_c = (n + 1e-10) * (1.0 + 0j)
        ...
        # Verify that energy_of_W matches the manual sum.
        out = scf_ns["energy_of_W"](
            W, h_like_atoms, ionic_potential, Eewald
        )

        assert float(out) == pytest.approx(
            expected, rel=r_tol, abs=a_tol
        )


class TestSteepestDescent:
    """Tests for the steepest-descent SCF optimization."""

    def _scf_state(self, scf_ns, atoms, seed):
        """Construct an SCF state from a reproducible initial orbital."""
        ...
        # Evaluate the XC energy density and potential.
        ...
        # No Ewald contribution for the single-atom test.
        Eewald = 0.0
    ...
    def test_reduces_energy(self, scf_ns, h_like_atoms):
        # Steepest descent should reduce the total energy.
...
class TestRunSCF:
    """Tests for the complete end-to-end SCF calculation."""

    def test_smoke_h_like(self, scf_ns, h_like_atoms):
        # Check that the complete SCF calculation returns a finite energy.
        out = scf_ns["runSCF"](
            h_like_atoms,
            0.0,
            5,
